...
一个割的**容量**（capacity）定义为：$\mathop{\mathrm{cap}}(A, B) = \sum_{e \in E_A^-}c(e)$。
...
*证明*：我们只需证明：此时存在一个割$(A, B)$使得$\mathop{\mathrm{cap}}(A, B) \leq v(f) + m\Delta$。令$A$为$s$在$G_f(\Delta)$中能到达的点的集合，$B = V \setminus A$，则和之前证明类似地，有：
$$
\begin{align*}
v(f) &= \sum_{e \in E_A^-} f(e) - \sum_{e \in E_A^+} f(e) \\
